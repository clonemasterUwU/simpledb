...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if(result.isPresent()){
            ...
            Pair<DataBox,Long> re = result.get();
            keys.add(i,re.getFirst());
            children.add(i+1,re.getSecond());
            return splitNodeIfExceed(maxOrder);
        } else {
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
            float fillFactor) {
        Optional<Pair<DataBox,Long>> result = getChild(children.size()-1).bulkLoad(data,fillFactor);
        if(result.isPresent()){
            int maxOrder = metadata.getOrder();
            Pair<DataBox,Long> re = result.get();
            keys.add(re.getFirst());
            children.add(re.getSecond());
            return splitNodeIfExceed(maxOrder);
        } else {
            return Optional.empty();
        }
    ...
    private Optional<Pair<DataBox, Long>> splitNodeIfExceed(int maxOrder) {
        if(keys.size()>2*maxOrder){
            assert(keys.size()==2*maxOrder+1);
            List<DataBox> newKeys = new ArrayList<>(keys.subList(maxOrder+1,2*maxOrder+1));
            List<Long> nextChildren = new ArrayList<>(children.subList(maxOrder+1,2*maxOrder+2));
            InnerNode splitNode = new InnerNode(metadata,bufferManager,newKeys,nextChildren,treeContext);
            DataBox ret = keys.get(maxOrder);
            keys.subList(maxOrder,2*maxOrder+1).clear();
            children.subList(maxOrder+1,2*maxOrder+2).clear();
            sync();
            splitNode.sync();
            return Optional.of(new Pair<>(ret,splitNode.page.getPageNum()));
        } else {
            sync();
            return Optional.empty();
        }
