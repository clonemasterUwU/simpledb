...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        List<DataBox> temp = leaf.getKeys();
        int i = 0;
        while (i < temp.size() && key.compareTo(temp.get(i)) > 0) i++;
    ...
    private void rootSplit(Pair<DataBox, Long> res) {
        List<DataBox> newKeys = Collections.singletonList(res.getFirst());
        List<Long> newChildren = Arrays.asList(root.getPage().getPageNum(), res.getSecond());
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        BPlusTreeIterator() {
            current = root.getLeftmostLeaf();
            while (current != null && current.getRids().size() == 0) {
                current = current.getRightSibling().orElse(null);
            }
            curIdx = current != null ? 0 : null;
        }

        BPlusTreeIterator(LeafNode current, int curIdx) {
            if (current == null || current.getRids().size() <= curIdx) {
                this.curIdx = null;
            } else {
        ...
        public RecordId next() {
            ...
            RecordId temp = current.getRids().get(curIdx);
            if (++curIdx == current.getRids().size()) {
                prefetch();
            }
            return temp;
        }
