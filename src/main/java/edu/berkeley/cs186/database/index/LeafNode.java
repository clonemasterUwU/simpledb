...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.size() >= 2 * maxOrder) { //resize
            assert(keys.size()==2*maxOrder);
            List<DataBox> newKeys = new ArrayList<>(maxOrder + 1);
            List<RecordId> newRids = new ArrayList<>(maxOrder + 1);
            if (key.compareTo(keys.get(maxOrder - 1)) < 0) {
                for (int i = maxOrder - 1; i < 2 * maxOrder; i++) newKeys.add(keys.get(i));
                for (int i = maxOrder - 1; i < 2 * maxOrder; i++) newRids.add(rids.get(i));
                keys.subList(maxOrder - 1, 2 * maxOrder).clear();
                rids.subList(maxOrder - 1, 2 * maxOrder).clear();
                int i = 0;
                while (i < maxOrder - 1 && keys.get(i).compareTo(key) < 0) {
                    i++;
                }
                if (i < maxOrder - 1 && keys.get(i).compareTo(key) == 0)
                    throw new BPlusTreeException("duplicated tree");
                keys.add(i, key);
                rids.add(i, rid);
            } else {
                for (int i = maxOrder; i < 2 * maxOrder; i++) newKeys.add(keys.get(i));
                for (int i = maxOrder; i < 2 * maxOrder; i++) newRids.add(rids.get(i));
                keys.subList(maxOrder, 2 * maxOrder).clear();
                rids.subList(maxOrder, 2 * maxOrder).clear();
                int i = 0;
                while (i < maxOrder && newKeys.get(i).compareTo(key) < 0) {
                    i++;
                }
                if (i < maxOrder && newKeys.get(i).compareTo(key) == 0) throw new BPlusTreeException("duplicated tree");
                newKeys.add(i, key);
                newRids.add(i, rid);
            }
            LeafNode next = new LeafNode(metadata, bufferManager, newKeys, newRids, this.rightSibling, treeContext);
            long nextPage = next.page.getPageNum();
            this.rightSibling = Optional.of(nextPage);
            sync();
            next.sync();
            return Optional.of(new Pair<>(newKeys.get(0), nextPage));
        } else {
            int i = 0;
            while (i < keys.size() && keys.get(i).compareTo(key) < 0) {
                i++;
            }
            if (i < keys.size() && keys.get(i).compareTo(key) == 0) throw new BPlusTreeException("duplicated tree");
            keys.add(i, key);
            rids.add(i, rid);
            if(keys.size()!=rids.size()){
                System.out.println();
            }
            sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                  float fillFactor) {
        int maxOrder = (int) Math.ceil(metadata.getOrder()*2*fillFactor);
        while(keys.size() < maxOrder &&  data.hasNext()){
            Pair<DataBox,RecordId> nxt = data.next();
            keys.add(nxt.getFirst());
            rids.add(nxt.getSecond());
        }
        sync();
        if(data.hasNext()){
            Pair<DataBox,RecordId> nxt = data.next();
            LeafNode nxtSib = new LeafNode(metadata,bufferManager, Collections.singletonList(nxt.getFirst()),Collections.singletonList(nxt.getSecond()),Optional.empty(),treeContext);
            long nxtPage = nxtSib.page.getPageNum();
            this.rightSibling = Optional.of(nxtPage);
            return Optional.of(new Pair<>(nxt.getFirst(),nxtPage));
        } else {
            return Optional.empty();
        }
    }
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        return new LeafNode(metadata, bufferManager, page, keys, rids, rightSibling == -1 ? Optional.empty() : Optional.of(rightSibling), treeContext);
    }
