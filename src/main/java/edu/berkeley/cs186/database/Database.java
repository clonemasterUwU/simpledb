...
public class Database implements AutoCloseable {
    ...
    private class TransactionContextImpl extends TransactionContext {
        ...
        public void close() {
            try {
                List<Lock> l = lockManager.getLocks(this);
                Collections.reverse(l);
//                l.sort((a, b) -> a.name.isDescendantOf(b.name) ? -1 : b.name.isDescendantOf(a.name) ? 1 : 0);

                for(Lock lock:l){
                    LockContext.fromResourceName(lockManager,lock.name).release(getTransaction());
                }
            } catch (Exception e) {
