...
public class SortOperator extends QueryOperator {
    ...
    public int estimateIOCost() {
        ...
        double pass0Runs = Math.ceil(1.0 * N / numBuffers);
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        PriorityQueue<Pair<Record,Integer>> pq = new PriorityQueue<>((a,b)->comparator.compare(a.getFirst(),b.getFirst()));
        List<Iterator<Record>> iterators = new ArrayList<>(runs.size());
        for(Run run:runs){
        ...
        while(!pq.isEmpty()){
            Pair<Record,Integer> nxt = pq.remove();
            temp.add(nxt.getFirst());
            int idx = nxt.getSecond();
            if(iterators.get(idx).hasNext()){
                pq.add(new Pair<>(iterators.get(idx).next(),idx));
            }
        }
        return temp;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for(int i=0;i<runs.size();i+=numBuffers-1){
            res.add(mergeSortedRuns(runs.subList(i,Math.min(i+numBuffers-1, runs.size()))));
        }
        return res;
    }
    ...
    public Run sort() {
        ...
        List<Run> first = new ArrayList<>();
        while(sourceIterator.hasNext()) {
            Iterator<Record> temp = QueryOperator.getBlockIterator(sourceIterator,getSchema(),numBuffers);
            first.add(sortRun(temp));
        }
        while(first.size()>1){
            first = mergePass(first);
        }
        return first.get(0);
    }
