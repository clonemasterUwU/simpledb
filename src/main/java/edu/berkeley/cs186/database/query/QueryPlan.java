...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int exclude = -1;
        for (Integer i : idxColumn) {
            SelectPredicate sp = selectPredicates.get(i);
            IndexScanOperator temp = new IndexScanOperator(transaction, sp.tableName, sp.column, sp.operator, sp.value);
            if (temp.estimateIOCost() < minOp.estimateIOCost()) {
                minOp = temp;
                exclude = i;
            }
        }
        return addEligibleSelections(minOp, exclude);
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Map.Entry<Set<String>, QueryOperator> prevJoin : prevMap.entrySet()) {
            for (JoinPredicate p : this.joinPredicates) {
                boolean containLeft = prevJoin.getKey().contains(p.leftTable);
                boolean containRight = prevJoin.getKey().contains(p.rightTable);
                if (containLeft && !containRight) {
                    Set<String> join = new HashSet<>(prevJoin.getKey().size() + 1) {{
                        add(p.rightTable);
                    }};
                    QueryOperator rightVal = pass1Map.get(join);
                    join.addAll(prevJoin.getKey());
                    QueryOperator total = minCostJoinType(prevJoin.getValue(), rightVal, p.leftColumn, p.rightColumn);
                    result.compute(join, (k, v) -> v == null ? total : (v.estimateIOCost() > total.estimateIOCost() ? total : v));
                }
                if (!containLeft && containRight) {
                    Set<String> join = new HashSet<>(prevJoin.getKey().size() + 1) {{
                        add(p.leftTable);
                    }};
                    QueryOperator leftVal = pass1Map.get(join);
                    join.addAll(prevJoin.getKey());
                    QueryOperator total = minCostJoinType(prevJoin.getValue(), leftVal, p.rightColumn, p.leftColumn);
                    result.compute(join, (k, v) -> v == null ? total : (v.estimateIOCost() > total.estimateIOCost() ? total : v));
                }
            }
        }
    ...
    public Iterator<Record> execute() {
        this.transaction.setAliasMap(this.aliases);
        Map<Set<String>, QueryOperator> joinPlan = new HashMap<>();
        for (String table : tableNames) {
            joinPlan.put(Set.of(table), minCostSingleAccess(table));
        }
        Map<Set<String>, QueryOperator> firstPass = new HashMap<>(joinPlan);
        while (joinPlan.size() > 1) {
            joinPlan = minCostJoins(joinPlan, firstPass);
        }
        finalOperator = minCostOperator(joinPlan);
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
