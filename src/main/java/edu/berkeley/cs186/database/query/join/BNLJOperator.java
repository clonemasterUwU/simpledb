...
public class BNLJOperator extends JoinOperator {
    ...
    private class BNLJIterator implements Iterator<Record> {
        ...
        private Record fetchNextRecord() {
            while (leftRecord == null && leftSourceIterator.hasNext()) fetchNextLeftBlock();
            if (leftRecord != null) {
                while(true) {
                    assert(leftRecord!=null);
                    while (rightPageIterator.hasNext()) {
                        Record right = rightPageIterator.next();
                        if (compare(leftRecord, right) == 0) {
                            return leftRecord.concat(right);
                        }
                    }
